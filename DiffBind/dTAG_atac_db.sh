...
# Modified from Nick's Version by preventing Summits from being calculated
...
#Setup fold change and FDR cutoffs
FDR=0.05
FC=0.58 #Log2(1.5)
...
cat >${subfolder}/${samp}_over_${control}_${normType}_$dbObj'.R' <<EOF
# Load libraries
library(DiffBind)
library(dplyr)
library(ggplot2)
...
#   dbObj <- dba.count(dbObj, bUseSummarizeOverlaps=TRUE, summits=FALSE, bParallel=FALSE)  # Does not run in parallel due to file size
...
dbObj <- dba.count(dbObj, bUseSummarizeOverlaps=TRUE, summits=FALSE, bParallel=TRUE)
dbObj <- dba.normalize(dbObj, method = DBA_ALL_METHODS, normalize=${norm}, offsets=${off})
...
unchanged <- out %>%
  filter(FDR > ${FDR} | abs(Fold) < ${FC}) %>% 
  select(seqnames, start, end)
write.table(unchanged, file="${subfolder}/unchanged.bed", sep="\t", quote=F, row.names=F)

allDB <- out %>% 
  filter(abs(Fold) > ${FC} & FDR < ${FDR}) %>%
...
numConstant=nrow(unchanged)
numGained=nrow(Gained)
numLost=nrow(Lost)

write.table(Gained, file="${subfolder}/upregulated.bed", sep="\t", quote=F, row.names=F, col.names=F)
write.table(Lost, file="${subfolder}/downregulated.bed", sep="\t", quote=F, row.names=F, col.names=F)
...
ggvolc <- function(data1,
  ...
  p <- ggplot2::ggplot(dplyr::arrange(dat1.2, threshold)) +
    ggplot2::geom_point(aes(x = Fold, y = log10FDR, color = threshold, size = size_aes),
                        shape = 16, alpha = 0.5) +
    ggplot2::theme_bw() +
    ggplot2::labs(title = "DESEQ2 ${samp} over ${control}",
                  x = "Fold",
                  y = "-log10(FDR)") +
    ggplot2::scale_color_manual(
      ...
      labels = c(paste0("Downregulated (",numLost,")"), paste0("non-significant (",numConstant,")"), paste0("Upregulated (",numGained,")"))
    )  +
...
res_deseq <- res_deseq %>% 
	mutate( threshold_KD = case_when(Fold > ${FC} & FDR <= ${FDR} ~ "Upregulated",
	Fold < -${FC} & FDR <= ${FDR} ~ "Downregulated",
	TRUE ~ "Unchanged"))
...
Rscript ${subfolder}/${samp}_over_${control}_${normType}_$dbObj'.R'
