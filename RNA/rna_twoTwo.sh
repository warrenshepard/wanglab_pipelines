...
cat >${folder}/twoFactorComparison/${treatment}_over_${control}_deseq/${treatment}_over_${control}_deseq'.R' <<EOF
  ...
  write.table(res_tableKD_tb,file="${treatment}_${control}_all.csv",row.names = TRUE,quote = FALSE,sep = ",")
...
all <- res_tableKD_tb
colnames(all)[colnames(all) == "gene"] <- "genes"
volcano <- ggvolc(all)
ggsave(plot = volcano, filename = "${treatment}_${control}_all_volcano.pdf")
